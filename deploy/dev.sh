...
cast send $FWB_TOKEN_ETHEREUM "mint(address,uint256)" $DEPLOYER_ADDRESS 9000000000000000000000000 --private-key $DEPLOYER_PRIVATE_KEY --rpc-url http://127.0.0.1:8545
BALANCE=$(cast call $FWB_TOKEN_ETHEREUM "balanceOf(address)" $DEPLOYER_ADDRESS --private-key $DEPLOYER_PRIVATE_KEY --rpc-url http://127.0.0.1:8545)
